// Fixed, Pure JavaScript MD5 Implementation (Requires zero external npm packages)
function md5(string: string) {
  ...
  let i;
  let str = unescape(encodeURIComponent(string)), l = str.length, n = (l + 8 >> 6) + 1, d: number[] = [];
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (let j = 0; j < 64; ++j) {
      ...
      else if (j < 32) { f = (a[1] & a[3]) | (a[2] & ~a[3]); g = (5 * j + 1) % 16; }
